...
from d7_factory_studio.features.diagnostics.node_params import build_node_plan
...
def test_commands_use_evt_timing_and_high_risk_gate() -> None:
    ...
    with pytest.raises(PermissionError, match="默认禁用"):
        clear_dmesg_request()
    assert clear_dmesg_request(high_risk_confirmed=True).argv == ("sudo", "dmesg", "-C")
...
def test_node_and_broadcast_plans_use_evt_catalog() -> None:
    evt = load_builtin_evt("EVT2")
    plan = build_node_plan(evt, "/opt/actuator_sdk/tool", logic_ids=(7,), write_then_read=True)
    ...
    assert f"{evt.broadcast.request_id:03X}##1" in request.argv[-1]
    assert responding_node(evt, "can2", evt.broadcast.response_base_id + 0x51).logic_id == 23
    summary = summarize_responses(evt, "can2", ("(1.0) can2 151##1AABB", "(1.1) can2 158##1CCDD"))
    assert set(summary["responses"]) == {23, 30}
    assert summary["response_frame_count"] == 2
